#!/usr/bin/env python3
"""
Test Generation CLI — Structural Model generation from functional specifications.

Usage:
    test_generation --generate --input spec.md --api-key "sk-..." --model openai/gpt-4o-mini
"""
...
from test_generation.framework.agents.base import set_max_concurrency
from test_generation.framework.orchestrator.generator import TestGenerationPipeline
from test_generation.framework.orchestrator.runs import (
    make_run_id,
    make_run_metadata,
    read_sidecar,
    write_sidecar,
...
def main():
    parser = argparse.ArgumentParser(
        description="Test Generation — Structural Model generation from functional specifications"
    )
    parser.add_argument("--version", action="version", version=f"test_generation {test_generation.__version__}")
    ...
    parser.add_argument(
        "--model",
        default=os.environ.get("MODEL", "openai/gpt-5-mini"),
        help="LiteLLM model string with provider prefix (default: openai/gpt-5-mini, overridable by MODEL env var)",
    )
    ...
    parser.add_argument("--debug", action="store_true", default=os.environ.get("DEBUG", "").lower() == "true", help="Enable debug logging (or set DEBUG=True in .env)")
...
def _generate(args):
    ...
    if not resume:
        if args.output != "output":
            ...
            output_dir = str(Path("agent_logs") / "test_generation" / project_slug / model_slug)
    ...
    generator = TestGenerationPipeline(
