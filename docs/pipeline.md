# Pipeline Walkthrough — Test Case Generation

This document traces a single execution of the full Test Generation Pipeline for a concrete module: **"Clients"** from the Mifos spec.
...
model:     openai/gpt-4o
output:    outputs/test_case_generation/Mifos/openai-gpt-4o/
```

The CLI parses the markdown and extracts 6 modules. "Clients" is module `id=1`.

---

## Stage 1 — Structural Model Generation

### Attempt 1: StructuralModelGeneratorAgent
...
### Attempt 1: StructuralModelValidatorAgent
...
```json
{
  ...
  "missing": [
    "Create_Client_Wizard.steps[2] — Address Details step with fields",
    "Create_Client_Wizard.steps[3] — Family Members step",
    "Create_Client_Wizard.steps[4] — Identifiers step (Document Type, Document Key)"
  ],
  "phantoms": [],
  "fixes": [
...
### Attempt 2: StructuralModelGeneratorAgent (with fixes)

The `fixes[]` array is appended to the prompt:

```
**CRITIC FEEDBACK FROM PREVIOUS ATTEMPT — address every point:**
1. Add wizard step 2 'Address Details' (description says step 2 covers address details)
2. Add wizard step 3 'Family Members'
3. Add wizard step 4 'Identifiers' with fields: Document_Type (dropdown), Document_Key (unspecified)
```

**Verdict: `yes`** — All 4 wizard steps now present. AST ships.
...
Receives the approved AST + description. Enumerates:
...
**Verdict: `yes`** — All state × action pairs covered, no phantoms.
...
## Stage 3 — Test Case Generation (Parallel)

Three agents run simultaneously for the "Client Management" module.

### PositiveTestCaseGeneratorAgent

Receives the `<workflows>` block appended to the prompt:
...
Produces 10 positive TCs — one covering every `wf_id`.
...
- WF-008: Transfer client to the same office → blocked (spec says same office is blocked)
- WF-009: Close client with active loan account → error, closure rejected
...
```
outputs/test_case_generation/Mifos/openai-gpt-4o/
...
## Post-Verification (Separate Run)
...
test-case-generation --post-verify \
  ...
  --test-cases outputs/.../test-cases.json \
  --api-key "$OPENAI_API_KEY"
```

For TC-004 (Create Client wizard → Submit):

1. **StateChangeAgent** returns `requires_post_verification: true` — creating a client is a write operation.
2. **PostVerificationAgent** generates:
...
```json
{
  ...
  "body": {
    "pre_check": {
      "navigate_to": "Clients page",
      "observe": ["total client count in table"]
    },
    "post_check": {
      "navigate_to": "Clients page",
      "observe": ["total client count in table", "new client row"],
      "expected_change": "Client count increases by 1; new client appears in table with Pending status."
    }
  }
}
```
