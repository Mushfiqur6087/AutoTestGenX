# Data Models — Test Case Generation
...
`forced_ship: true` means the module hit the 3-attempt cap. The final attempt's output was shipped regardless of the critic verdict. Inspect `critique.missing` and `critique.fixes` to understand what to address in the spec or prompt.
...
```json
{
  ...
  "body": {
    ...
    "post_check": {
      "navigate_to": "Accounts Overview",
      "observe": ["balance of source account", "balance of destination account"],
      "expected_change": "Source balance decreased; destination balance increased by transfer amount."
    }
  }
}
```
...
```json
{
  ...
  "body": {
    ...
    "actor_b": {
      ...
      "expected_change": "Assignment appears with correct due date and status 'No submission'."
    }
  }
}
```
...
| Type | When used |
|------|-----------|
| `same_actor_navigation` | Same user navigates elsewhere to observe the state change |
| `cross_actor` | Change performed by Actor A, verified by Actor B in a separate session |
| `other` | Effect is outside the app boundary (e.g. external bank); `coverage` = `partial` |

### Coverage values

| Value | Meaning |
|-------|---------|
| `verifiable` | Full state change is observable within the app |
| `partial` | Only part of the state change is observable |
| `unverifiable` | State change cannot be confirmed from any in-app page |
